Hooks:PostHook( WeaponTweakData, "init", "AKMSUInit", function(self)

--- TBC ---
-- if self.new_m4.reload_timed_stance_mod then
	-- self.ak_krink_762.reload_timed_stance_mod = {
		-- empty = {
			-- hip = {
...
if BeardLib.Utils:FindMod("Custom Categories for custom weapons") then
	self.ak_krink_762.categories = {"assault_rifle"}
else
	self.ak_krink_762.categories = {"smg"}
end

if ( self.ak_krink_762 ) then
	
	self:SetupAttachmentPoint("ak_krink_762", {
		name = "a_ns", 
        base_a_obj = "a_ns", 

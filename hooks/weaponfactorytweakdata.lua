Hooks:PostHook( WeaponFactoryTweakData, "init", "AKMSUInit", function(self)
...
local all_akmsuext = {
	...
	"wpn_fps_ass_ak_krink_762_md_akmsu"
}
...
if attach_tables then
	for _, md_id in pairs(attach_tables.Barrel_Extensions) do
		if self.parts[md_id] then
			table.insert(all_akmsuext, md_id)
		end
	end
...
for id, st_id in pairs (all_ar15stock) do
	self.wpn_fps_ass_ak_krink_762.override[st_id] = {depends_on = "stock_adapter"}
	self.wpn_fps_ass_ak_krink_762.override[st_id] = {a_obj = "a_s_ar15"}
end
...
for id, o_id in pairs(all_ak545sight) do
	self.wpn_fps_ass_ak_krink_762.adds[o_id] = {"wpn_fps_ass_ak_krink_762_om_tula"}
end
for id, md_id in pairs(all_akmsuext) do
	-- self.parts.wpn_fps_ass_ak_krink_762_ba_ak105.override[md_id] = {a_obj="a_ns_ak105"}
end
--=========================--
--          STANCE         --
--=========================--
...
for id, o_id in pairs(vanilla_ak545sight) do
	if self.parts[o_id].stance_mod.wpn_fps_ass_flint then
		self.parts[o_id].stance_mod.wpn_fps_ass_ak_krink_762 = deep_clone(self.parts[o_id].stance_mod.wpn_fps_ass_flint)
	else
